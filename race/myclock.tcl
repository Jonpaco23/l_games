	...
	set cy [expr $h /2]
   	...
   	set fnt2Size 0.06	
           ...
           .clock create line 100 100 100 5  -width 9 -tags {hands secondhand} \
                   -fill red -capstyle round -smooth true	   place .clock -in . -relx 0.055 -rely 0.37
   ...
   proc updateClock {} {
	   ...
	   set sp [expr {double($mob(gates_passed))/double($mob(endgame))}]
	   #puts "sp = $sp ob(gates_deleted) =$ob(gates_deleted) endgame = $mob(endgame)"
           #move the circle
