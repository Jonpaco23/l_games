...
proc make_fixed_list {rom} {
	...
	if {$ob(fitts,var)==2 || $ob(fitts,var)==3} {
		...
		if {$ind%2==0} {
				#for random do $rand_dir ==0, 1 for the next
				if {$ob(fitts,speed) == 2} {
...
proc do_drag {w} {
	...
	if {$ob(fitts,var)<=3} {
		if {($x >= -0.001 && $x <= 0.001 && $ob(hdir) == 1) || ($y >= -0.001 && $y <= 0.001 && $ob(hdir) == 0) || ($ob(hdir) == 2 && $ob(cross)%2 == 0 && $x >= -0.01 && $x <= 0.01) || ($y >= -0.01 && $y <= 0.01 && $ob(hdir) == 2 && $ob(cross)%2 == 1)} {
			if {$ob(two) != 1 && $ob(zero) == 1} {
				set ob(two) 1
				puts "Trigger 7 sent! I am in the middle!"
	...
	if {[expr {abs($vel)}]<0.001} {
		if { $ob(zero) == 1 && $ob(one) == 1 && $ob(two) == 1 && $ob(three) == 1} {
			if {($x >= -0.01 && $x <= 0.01 && $ob(hdir) == 1) || ($y >= -0.01 && $y <= 0.01 && $ob(hdir) == 0) || ($ob(hdir) == 2 && $ob(cross)%2 == 0 && $x >= -0.01 && $x <= 0.01) || ($y >= -0.01 && $y <= 0.01 && $ob(hdir) == 2 && $ob(cross)%2 == 1)} {
				incr ob(numTrials)
				set ob(zero) 0
				set ob(one) 0
				set ob(two) 0
...
proc show_target {} {
	...
	if {$k >= [expr {$ob(endgame)/2.}]} {
		puts "Trigger 11 sent! The experiment has ended"
	...
	if {$ob(targets_created) == 0} {
		puts "Trigger 1 sent! The experiment has begun"
		set command [exec python TriggerTests.py 1]
		puts $command
	} else {
		puts "Trigger 13 sent! New target has appeared"
	...
	if {$hit > 0} {
		...
		if {$ob(tarNum) == 0 || $ob(tarNum) == 1 || $ob(tarNum) == 2} { 
			if {$ob(zero) != 1} {
				set ob(zero) 1
				if {$ob(hdir) == 1 || ($ob(hdir) ==2 && $ob(cross)%2==0)} {
					puts "Trigger 5 sent! Left target has been hit"
					set command [exec python TriggerTests.py 5]
					puts $command
				} else {
					puts "Trigger 15 sent! Down target has been hit"
				...
				set ob(one) 1
				if {$ob(hdir) == 1 || ($ob(hdir) ==2 && $ob(cross)%2==0)} {
					puts "Trigger 9 sent! Right target has been hit"
					set command [exec python TriggerTests.py 9]
					puts $command
				} else {
					puts "Trigger 17 sent! Up target has been hit"
