...
proc make_fixed_list {rom} {
	...
	#This is for "random" Reaction times following ex-Gaussian distribution
	set fp [open "rt.txt" r]
...
proc show_target {} {
	...
	if {$ob(fitts,var)==3} {
		after 200
		set ob(reaction_time) [lindex $ob(rt_list) $i]	
		if {$ob(tarNum) == 0} {
			#This is for min (zero)
			after $ob(reaction_time) 
			moveArm $ob(previous) -0.20
			set ob(previous) -0.20
		} elseif {$ob(tarNum) == 7} {
			#This is for max (7)
			after $ob(reaction_time)
			moveArm $ob(previous) 0.20
