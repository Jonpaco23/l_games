...
proc init_Fitts {} {
	...
	set ob(numTrials1) 1
	set ob(numTrials2) 2
	set ob(triggerCount) 0
	
	set ob(totTrials) [expr 3*$ob(numTrials1) + 3*$ob(numTrials2)]
...
proc do_drag {w} {
	...
	if {$ob(fitts,var)<=4} {
		...
		#puts "VALUE OF CROSS BEFORE DRAG"
		#puts $ob(cross)
		if {$ob(hdir) == 1 || ($ob(hdir) ==2 && $ob(cross)%2==0)} {
			dragx $w $x racer	
	    	...
	    	if { ($ob(leftOrDownFlag) == 1 || $ob(rightOrUpFlag) == 1)} {
				...
				if {$ob(curTrials)==$ob(expTrials)} {
					...
					if {!($ob(changeMode)==1 && $ob(totalModes) ==6)} {
						...
						tk_dialog .dialog1 "Break Time !" "Five Second break" info 0 ""
					}
...
proc domenu {} {
	...
	menu_t $m blank18 "     These are instructions" ""
	#menu_v $m test "Number of Trials per Motion" 20
