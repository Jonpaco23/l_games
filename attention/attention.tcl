...
# this array goes towards the higher demands
proc set_array_1 {} {
	global a
...
# this array changes one parameter per session
proc set_array_2 {} {
	global a
	set a(1,1) 1
	set a(1,2) 2
...
# this array samples fast the highest demands
proc set_array_2 {} {
	global a
	set a(1,1) 1
	set a(1,2) 10
...
proc select_new_parameters {} {
	global ob a

	for { set k  1} { $k <=5 } { incr k } {
		for { set m 1 } { $m <= 5} { incr m } {
			if { $a($k,$m) == $ob(index_param) } {
				set ob(bspeed) [lindex $ob(lbspeed) [expr {$m-1}]] 
				set ob(padw) [lindex $ob(lpaddlew) [expr {$k-1}]] 
				puts "k = $k, m = $m"
			}	
		}
	}
...
proc adjust_adap_controller {} {
	...
	puts "Bounce $mob(bounces) size = $ob(padw) , speed = $ob(bspeed)"
...
# create menu buttons
	...
	set ob(lbspeed) [list 0.5 1.25 2.0 2.75 3.5]
	set ob(bspeed) [lindex $ob(lbspeed) 0] 
	...
	set ob(lpaddlew) [list 140 110 80 50 20]
	set ob(padw) [lindex $ob(lpaddlew) 0] 
...
proc new_pong {} {
	...
	# this is for selecting the right parameter to change every 10 bounces, according to the protocol
	set ob(index_param) 1
...
# change these to get input from arm bot
proc getxy {} {
	global ob
...
proc prepare_logging {} {
	...
	set ob(gamename) games/exper/attention_log
	...
	if {$ob(planar)} {
    	...
    	set ob(dirname) [file join $ob(logdirbase) $ob(patname) $ob(gametype) $ob(datestamp) attention ]
...
proc domenu {} {
	...
	menu_v $m endgame "Game Length" 250
	menu_v $m padw "Paddle Width (1-5)" 1
	menu_v $m level "Speed Level (1-5)" 1
...
proc do_once {} {
	...
	set ob(n_games) 0
	# by default, south wall is live
	set mob(whichgame) s
	# protocol array (determines the order of changing game parameters (speed, accuracy demand))
	set_array_1
}
