...
proc show_target {} {
	...
	puts "Trigger 4 sent! New target has appeared"
...
proc check_target {} {
	...
	if {$hit > 0} {
		...
		if {$ob(tarNum) == 0 || $ob(tarNum) == 1 || $ob(tarNum) == 2} { 
			if {$ob(zero) != 1} {
				set ob(zero) 1
				puts "Trigger 0 sent! Left/Down target has been hit"
			    ...
			    if {$ob(one) != 1} {
				set ob(one) 1
				puts "Trigger 1 sent! Right/Up target has been hit"
