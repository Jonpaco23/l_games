...
proc make_fixed_list {rom} {
	...
	if {0} {
		for {set ind 0} {$ind < [expr 1.5*{$ob(endgame)}]} {incr ind} {	
...
proc size_racer {} {
	...
	.c scale helper 0 0 [expr $ob(scale)*6] [expr -$ob(scale)*6]
...
proc do_drag {w} {
	...
	if {$vel >= 0.0015} {
		if {$ob(three) != 1 && $ob(numTrials) != $ob(endgame)/2.} {
		    #set timeID [expr {abs([rshm i])}]
		    #puts $timeID
		    #lappend ob(triggerList) 3
		    #lappend ob(timeIDList) $timeID 
	            wshm event_id 1
		    set command [exec python TriggerTests.py 1]
		    #set ob(eventID) 3
		    puts $command
		    puts "Trigger 1 sent! I am moving!"
		    #puts $vel
		    set ob(three) 1
		}
	} 
	...
	puts "HERE"
	set stuff [eval .c coords helper] 
	puts $stuff
	set helpC [lindex $stuff 0]
	puts $helpC
	.c move helper 0.01 0 
	...
	if {$ob(fitts,var)<=3} {
		if {($x >= -0.001 && $x <= 0.001 && $ob(hdir) == 1) || ($y >= -0.001 && $y <= 0.001 && $ob(hdir) == 0) || ($ob(hdir) == 2 && $ob(cross)%2 == 0 && $x >= -0.01 && $x <= 0.01) || ($y >= -0.01 && $y <= 0.01 && $ob(hdir) == 2 && $ob(cross)%2 == 1)} {
			if {$ob(two) != 1 && ($ob(zero) == 1 || $ob(one)==1)} {
				set ob(two) 1
				#set ob(eventID) 7
				#set timeID [expr {abs([rshm i])}]
			        #puts $timeID
			        #lappend ob(trigggerList) 7
			        #lappend ob(timeIDList) $timeID 
				wshm event_id 3
...
proc show_target {} {
	...
	.c raise helper target
	
	if {$ob(targets_created) == 0} {
		#set ob(eventID) 1
		#set timeID [expr {abs([rshm i])}]
	        #puts $timeID
	        #lappend ob(triggerList) 1
	        #lappend ob(timeIDList) $timeID 
		wshm event_id 0
		set command [exec python TriggerTests.py 0]
		puts $command
		puts "Trigger 0 sent! The experiment has begun"
	} else {
		#set ob(eventID) 13
		#set timeID [expr {abs([rshm i])}]
	        #puts $timeID
	        #lappend ob(triggerList) 13
	        #lappend ob(timeIDList) $timeID 
		wshm event_id 7
...
proc check_target {} {
	...
	puts "HERE2"
	puts $ob(left_frame)
	puts [lindex [eval .c find overlapping $bbox] 1]
...
proc do_title {} {
	...
	wm title . "EEG Game  ||  Name: $mob(whoN)  ||  Total Targets: $ob(endgame)  ||  Cleared Targets: $ob(targets_hit) || Total Trials: $ob(test) || Cleared Trials: $ob(numTrials)"
}
...
proc init_logging {} {
	...
	puts $ob(log_fd) "Type: $ob(whichgame)"
    	puts $ob(log_fd) "Gametype: EEG experiment"
	...
	puts $ob(log_fd) "Target Widths: "
	puts $ob(log_fd) "$ob(ltarget_width)"
	puts $ob(log_fd) "Targets List:" 
	puts $ob(log_fd) "$ob(targets_list)" 
	puts $ob(log_fd) "Velocity:" 
	puts $ob(log_fd) "$ob(targets_list) speed"
	puts $ob(log_fd) "Direction:" 
	puts $ob(log_fd) "$ob(targets_list) direction"
    	flush $ob(log_fd)
}
...
proc init_logging2 {} {
	global ob mob
    	...
    	file mkdir $ob(dirname)
	puts $filename
    	set ob(log_fd) [open "${filename}SECOND.asc" w]
    	puts $ob(log_fd) "Time: $ob(timestamp)"
    	puts $ob(log_fd) "Name: $mob(whoN)"
    	puts $ob(log_fd) "Dir: $ob(dirname)"
    	puts $ob(log_fd) "Game: $tailname"
	puts $ob(log_fd) "Type: $ob(whichgame)"
    	puts $ob(log_fd) "Trigger List:"
	puts $ob(log_fd) "$ob(triggerList)"
	puts $ob(log_fd) "TimeID List:" 
	puts $ob(log_fd) "$ob(timeIDList)" 
    	flush $ob(log_fd)
}
